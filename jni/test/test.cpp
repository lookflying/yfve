...
TEST(msg_test, dump_register){
				string csimcardnum = "15965253644";
				MSG_BYTE* numbers;
				string2num(csimcardnum, &numbers);
				set_global_phone_num(numbers, csimcardnum.size());
				delete[] numbers;
				TerminalRegisterMessage msg;
				msg.provinceId = (MSG_WORD) 0;
				msg.cityId = (MSG_WORD) 0;
				string cmakerId = ""; 
				...
				msg.color = (MSG_BYTE) 0;
				msg.carPlate = "苏A0000";
				char* buf = msg.toBytes();
				vector<msg_serialized_message_t> packed;
				MSG_WORD serial;
				EXPECT_EQ(true, pack_msg(0X0100, buf, 0, msg.len(), packed, serial));
				delete[] buf;
