...
void clear_unpack_cache(const MSG_WORD msg_seq) {
	...
	if ((it = msg_g_unpack_cache.find(msg_seq)) != msg_g_unpack_cache.end()) {
		for (msg_it = it->second.begin(); msg_it != it->second.end();
				++msg_it) {
...
string header2string(const msg_header_t header) {
	...
	for (unsigned int i = 0; i < sizeof(header.phone_num) / sizeof(MSG_BCD);
			++i) {
...
bool serialize(const msg_message_t &message,
	...
	escaped_serialize_buf.insert(escaped_serialize_buf.begin(),
			(char) MSG_FLAG);
	...
	if (str.size() < MSG_HEADER_MIN_SIZE) {
		...
		if (MSG_IS_DIVIDED(header.property)) {
			if (str.size() >= MSG_HEADER_MAX_SIZE - MSG_HEADER_MIN_SIZE) {
				MSG_SET_WORD(header.pack_opt.pack_count, (MSG_BYTE)str.at(0),
						(MSG_BYTE)str.at(1));
				MSG_SET_WORD(header.pack_opt.pack_seq, (MSG_BYTE)str.at(2),
						(MSG_BYTE)str.at(3));
				str.erase(0, 4);
			} else {
				return false;
...
bool deserialize(const msg_serialized_message_t &serialized,
		msg_message_t &message) {
	string deserialize_buf = "";
	string unescaped_deserialize_buf = "";
	if ((char) MSG_FLAG == (char) (*serialized.data)
			&& (char) MSG_FLAG
					== (char) (*(serialized.data + serialized.length - 1))) {
		deserialize_buf.append((char*) (serialized.data + 1),
				serialized.length - 2);
		...
		if (deserialize_header(unescaped_deserialize_buf, temp_header)) {
			...
			if (unescaped_deserialize_buf.size() != msg_len + 1) {/*校验码占一个字节*/
				...
				if (message.body.content == NULL) {
					...
					memcpy(&message.body.content[0],
							unescaped_deserialize_buf.c_str(), msg_len);
...
msg_header_t generate_header(MSG_WORD id, MSG_WORD property, MSG_BCD *phone_num,
		MSG_WORD seq, msg_pack_opt_t pack_opt) {
	msg_header_t header;
	header.id = id;
...
bool pack_msg(const MSG_WORD id, const char* msg_data,
		const unsigned char encrypt, const unsigned int msg_len,
		vector<msg_serialized_message_t> &packed, MSG_WORD &msg_seq) {
	packed.resize(0);
	if ((MSG_MAX_PACK_SIZE == 0 || msg_len <= (unsigned int) MSG_MAX_PACK_SIZE)
			&& msg_len <= (unsigned int) MSG_PACK_SIZE_LIMIT) {
		MSG_WORD property = MSG_PACK_PROPERTY(false, encrypt, msg_len);
		msg_message_t temp_msg;
		msg_serialized_message_t serialized_msg;
		temp_msg.header = generate_header(id, property, msg_g_phone_num,
				msg_g_msg_seq);
		...
		if (MSG_MAX_PACK_SIZE == 0) {
			...
			divided_size =
					((unsigned int) MSG_MAX_PACK_SIZE
							< (unsigned int) MSG_PACK_SIZE_LIMIT) ?
							(unsigned int) MSG_MAX_PACK_SIZE :
							(unsigned int) MSG_PACK_SIZE_LIMIT;
		}
		unsigned int seq = 0;
		...
		while (remain > 0) {
			...
			msg_serialized_message_t serialized_msg;
			temp_msg.header = generate_header(id, property, msg_g_phone_num,
					msg_g_msg_seq, generate_pack_option(count, seq + 1));
			temp_msg.body.content =
					(MSG_BYTE*) (msg_data + (seq * divided_size));
			temp_msg.body.length = cur_size;
			if (!serialize(temp_msg, serialized_msg)) {
				for (vector<msg_serialized_message_t>::iterator it =
						packed.begin(); it != packed.end(); ++it) {
...
bool unpack_msg(const msg_message_t &msg, MSG_WORD &msg_id, MSG_WORD &msg_seq,
		char** msg_data, unsigned int &msg_len) {
	msg_len = 0;
	if (MSG_IS_DIVIDED(msg.header.property)) {
		map<MSG_WORD, map<MSG_WORD, msg_message_t> >::iterator it;
		map<MSG_WORD, msg_message_t>::iterator msg_it;
		if ((it = msg_g_unpack_cache.find(msg.header.seq))
				!= msg_g_unpack_cache.end()) {
			if (msg_g_unpack_id[msg.header.seq] != msg.header.id
					|| msg_g_unpack_count[msg.header.seq]
							!= msg.header.pack_opt.pack_count) {
		...
		if (it->second.size() == msg_g_unpack_count[msg.header.seq]) {
			*msg_data = new (nothrow) char[msg_g_unpack_size[msg.header.seq]];
			if (msg_data == NULL)
				return false;
			char* ptr = *msg_data;
			for (MSG_WORD i = 1; i <= msg_g_unpack_count[msg.header.seq]; ++i) {
				msg_it = it->second.find(i);
			...
			clear_unpack_cache(msg.header.seq);
			return true;
			index_not_found: delete[] *msg_data;
			return false;
